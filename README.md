...
基于 **Cloudflare Workers + D1** 的临时邮箱控制台。它会接收入站邮件，先按发件人白名单决定是否放行，再按规则提取主题和正文中的关键信息，把结果写入 D1，并通过后台控制台与 API 对外提供查询能力。当前版本还支持把原始邮件继续转发到已验证的真实邮箱，例如 QQ 邮箱，并内置一组常见提取规则作为兜底。
...
- 邮件通过白名单后，会对主题、纯文本正文、HTML 文本和原始 HTML 执行规则匹配，并保存命中的完整正则结果、内容摘要和命中前后上下文。
...
- 提取策略支持 `内置 + 自定义`、`只用内置规则`、`只用自定义规则` 三种模式。

## 功能

- 🧩 **控制台管理**：邮件列表检索、规则管理、白名单管理、转发设置与 API 对接说明。
- 🛡️ **发件人白名单**：基于正则表达式过滤发信人，不匹配直接忽略。
- 🔍 **内容正则提取**：对邮件主题 + 正文进行正则匹配，提取验证码、英文数字组合、连字符代码、链接和封禁通知等关键信息。
...
- **提取策略**：在控制台里切换“内置 + 自定义 / 只用内置规则 / 只用自定义规则”。
...
| `..rule_key` | `string` | 内置规则的固定标识；自定义规则时通常为 `null` |
| `..source` | `string` | `builtin` 或 `custom` |
...
| `pattern` | 内容提取正则，会匹配邮件主题 + 正文；自定义规则记录首个完整匹配，内置规则会抓取多项常见信号 |
